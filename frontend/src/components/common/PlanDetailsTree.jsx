...
import { Calendar, Wallet, Landmark, TrendingUp, Info } from "lucide-react"
import { formatCurrency } from "@/utils/formatters"

export function PlanDetailsTree({ plan }) {
  ...
  const rawSteps = plan.planDetails?.steps || [
    {
      ...
      note: `Số tiền ban đầu: ${formatCurrency(plan.initialDeposit)}`,
    },
    {
      month: 0,
      action: "open",
      ...
      rate_pct: plan.rate,
      note: `Gửi vào ${plan.bankName} kỳ hạn ${plan.term} tháng với lãi suất ${plan.rate}%/năm`,
    },
    {
      month: plan.term,
      action: "mature",
      amount: plan.initialDeposit + (plan.estimatedInterest || plan.interestEarned || 0),
      term: plan.term,
      bank_id: plan.bankCode,
      bank_name: plan.bankName,
      rate_pct: plan.rate,
      note: `Đáo hạn: gốc ${formatCurrency(plan.initialDeposit)} + lãi ${formatCurrency(plan.estimatedInterest || plan.interestEarned || 0)}`,
    }
  ]
  ...
  const renderBookNode = (node) => {
    ...
    return (
      ...
      >
        {level > 0 && (
          <div
            className={`absolute -left-6 top-[28px] w-6 border-t-2 border-dashed transition-colors duration-200 ${
              isHighlighted ? "border-indigo-500" : "border-slate-200"
            }`}
          />
        )}

        <div
          className={`border rounded-2xl bg-white transition-all duration-300 relative overflow-hidden w-[600px] shrink-0 ${
            isHighlighted
  ...
  return (
    <>
      {/* Initial state card */}
      {tree.initial && (
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-2xl p-5 shadow-sm text-center relative overflow-hidden mb-6">
            ...
            {tree.roots.map((rootNode, idx) => (
              ...
              >
                <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-3">
                  <h5 className="text-sm font-bold text-slate-800 flex items-center gap-2 m-0">
                    <span
      ...
      {tree.unmapped && tree.unmapped.map((step, idx) => (
        <div key={idx} className="bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-100 rounded-2xl p-5 shadow-sm text-center relative overflow-hidden mt-6">
